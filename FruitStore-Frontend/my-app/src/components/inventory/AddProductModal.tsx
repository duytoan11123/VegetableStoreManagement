...
import {Supplier, Category, InventoryItem, CreateItemData, AddProductModalProps, FormData} from '@/type/Inventory.types';
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onSuccessRefresh, suppliers, categories,itemToEdit }) => {
    const { token } = useAuth();
        ...
        if (isOpen) {
            if (isEditMode && itemToEdit) {
                setFormData({
                    ...
                    categoryId: categories.find(c => c.name === itemToEdit.categoryName)?.id || '',
                    supplierId: itemToEdit.supplierId,
                });
            } else {
                setFormData({
                    name: '', quantity: '', price: '', supplierId: '', categoryId: ''
                });
            }
            setSubmitError(null);
            setErrors({});
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 m-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="quantity" className="block text-sm font-medium text-gray-700">Số Lượng (kg)</label>
                            <input
                                type="number"
                                id="quantity"
                                name="quantity"
                        ...
                        <div>
                            <label htmlFor="price" className="block text-sm font-medium text-gray-700">Đơn Giá (VNĐ)</label>
                            <input
                                type="number"
                                id="price"
                                name="price"
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        ...
                        >
                            ...
                            {isLoading ? (isEditMode ? 'Đang cập nhật...' : 'Đang thêm...') : (isEditMode ? 'Lưu Thay Đổi' : 'Thêm Sản Phẩm')}
                        </button>
                    </div>
                </form>
            </div>
