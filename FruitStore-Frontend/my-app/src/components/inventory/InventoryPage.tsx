...
export default function InventoryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const fetchItems = async () => {
      ...
      if (!response.ok)
        throw new Error(`Lỗi tải Items: ${response.statusText}`);
    ...
    const fetchCategories = async () => {
      ...
      if (!response.ok)
        throw new Error(`Lỗi tải Categories: ${response.statusText}`);
      return response.json();
    };
    ...
    const fetchSuppliers = async () => {
      ...
      if (!response.ok)
        throw new Error(`Lỗi tải Suppliers: ${response.statusText}`);
      return response.json();
    };
  ...
  const kpiData = useMemo(() => {
    ...
    const totalQuantity = formatLargeNumber(
      items.reduce((sum, item) => sum + item.quantity, 0)
    );
    const lowStockCount = items.filter(
      (item) => item.status === "LOW" || item.status === "SOLDOUT"
    ).length;
    const totalValue = formatLargeNumber(
  ...
  const handleDelete = async (itemId: number, itemName: string) => {
    ...
    try {
      const response = await fetch(
        `${backendApiUrl}/inventory/items/${itemId}`,
        {
          method: "DELETE",
          headers: {
      ...
      <div className="bg-white p-4 rounded-xl shadow-lg mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex-grow w-full md:w-auto relative">
          ...
          <button
            ...
            className="w-full px-4 py-2 pr-10 text-left text-gray-500 border border-gray-300 rounded-lg bg-white focus:ring-green-500 focus:border-green-500 flex items-center justify-between hover:bg-gray-50 transition"
          >
            <span>
              {filterCategory
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {hasRole("ADMIN") && <th className="px-6 py-3"></th>}
            </tr>
          </thead>
        ...
        <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
          <div className="hidden sm:block">
            <p className="text-sm text-gray-700">
              ...
              <span className="font-medium">
                {pageData ? pageData.number * PAGE_SIZE + 1 : 0}
              </span>{" "}
              đến{" "}
              <span className="font-medium">
                {pageData
                  ? pageData.number * PAGE_SIZE + pageData.content.length
                  : 0}
              </span>{" "}
              trong tổng số{" "}
              <span className="font-medium">
          ...
          <div className="flex items-center gap-x-3">
            ...
            <span className="text-sm text-gray-900 font-medium px-4">
              Trang {pageData ? pageData.number + 1 : 0} /{" "}
