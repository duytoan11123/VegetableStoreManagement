...
import { Search, ChevronDown, ShoppingCart, UserSearch, CreditCard, DollarSign, Trash2, ShoppingBag, PackageX, X, Image as ImageIcon } from 'lucide-react';
        ...
        <div className="flex flex-col lg:flex-row gap-6 max-w-full">
            ...
            <div className="lg:w-2/3">
                <div className="flex justify-between items-center mb-6 mt-4 lg:mt-0">
                    ...
                    <Link
                        href="/orders/history"
                        className="flex items-center px-4 py-2 bg-white text-blue-600 border border-blue-200 rounded-lg font-medium hover:bg-blue-50 transition shadow-sm"
                    >
                ...
                <div className="bg-white p-4 rounded-xl shadow-lg mb-6 flex flex-col md:flex-row gap-4">
                    ...
                    <div className="relative flex-grow md:flex-grow-0 md:w-64">
                        <select
                            className="w-full pl-4 pr-10 py-2 border border-gray-300 rounded-lg text-gray-600 bg-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent appearance-none cursor-pointer hover:border-green-400 transition-colors"
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            ...
                            {products.map(product => (
                                <button
                                    ...
                                    className="hover:cursor-pointer border border-gray-200 rounded-xl p-4 text-left hover:shadow-lg transition-all duration-200 group hover:border-green-500 disabled:opacity-60 disabled:cursor-not-allowed bg-white flex flex-col h-full"
                                >

                                    <div className="h-16 w-full bg-gray-50 rounded-lg mb-3 flex items-center justify-center group-hover:bg-green-50 transition-colors duration-200">
                                        <ImageIcon className="w-16 h-16 text-black group-hover:text-green-300" />
                                    </div>

                                    <div className="font-semibold text-gray-800 truncate text-base mb-1" title={product.name}>
                                        {product.name}
                                    </div>

                                    <div className="flex flex-col items-start mt-auto w-full">
                                        <div className="text-green-600 font-bold text-ls">
                                            {product.price.toLocaleString('vi-VN')} <span className="text-xs">₫</span>
                                        </div>
                                        <div className={`text-xs font-medium  ${product.quantity > 0 ? ' text-gray-600' : ' text-red-600'}`}>
                                            Tồn kho: {product.quantity > 0 ? `${product.quantity} kg` : 'Hết'}
                                        </div>
                                    </div>
                                </button>
                            ))}
                        ...
                        {cart.map(item => (
                            <div key={item.id} className="py-3 flex items-center gap-3">
                                ...
                                <input
                                    ...
                                    min="0"
                                />
                                <button onClick={() => handleRemoveFromCart(item.id)} className="hover: cursor-pointer text-red-500 hover:text-red-700">
