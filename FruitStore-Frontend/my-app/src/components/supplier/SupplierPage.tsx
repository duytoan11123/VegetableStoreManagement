...
export default function SupplierPage() {
  ...
  const [selectedHistorySupplier, setSelectedHistorySupplier] = useState<{
    id: number;
    name: string;
  } | null>(null);

  // Hàm mở Modal
  const handleOpenHistory = (supplier: any) => {
    setSelectedHistorySupplier({ id: supplier.id, name: supplier.name }); // Đảm bảo đúng tên trường dữ liệu của bạn
  ...
  const displayedSuppliers = useMemo(() => {
    ...
    // Sắp xếp (an toàn với undefined/null và hỗ trợ số/chuỗi)
    sortedSuppliers.sort((a, b) => {
      const aVal = a[sort.field];
      const bVal = b[sort.field];

      // Cân nhắc trường hợp null/undefined
  ...
  const handleDelete = async (supplierId: number, supplierName: string) => {
    if (
      !window.confirm(
        `Bạn có chắc chắn muốn xóa NCC: ${supplierName}? (Hành động này có thể thất bại nếu NCC đang được liên kết với sản phẩm)`
      )
    ) {
      return;
    }
  ...
  return (
    <>
      ...
      <SupplierHistoryModal
        ...
        supplierId={selectedHistorySupplier?.id || null}
        supplierName={selectedHistorySupplier?.name || ""}
      />
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg overflow-x-auto">
                  ...
                  {hasRole("ADMIN") && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-x-3">
                      ...
                      <button
                        ...
                        className="hover:cursor-pointer text-blue-600 hover:text-blue-900"
                      >
        ...
        <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
          <p className="text-sm text-gray-700">
            Hiển thị tổng số{" "}
            <span className="font-medium">{displayedSuppliers.length}</span> kết
            quả
          </p>
        </div>
      </div>
