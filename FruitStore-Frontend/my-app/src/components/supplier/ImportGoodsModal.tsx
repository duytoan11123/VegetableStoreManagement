...
type ImportFormState = Record<string, {
...
const ImportGoodsModal: React.FC<ImportGoodsModalProps> = ({ 
        ...
        if (isOpen && token && supplierId) {
            ...
            const fetchItemsBySupplier = async () => {
                ...
                try {
                    ...
                    if (!response.ok) throw new Error('Không thể tải danh sách sản phẩm của NCC');
    ...
    const handleChange = (itemId: number, field: 'quantity' | 'price', value: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (itemsToImport.length === 0) {
            setSubmitError("Bạn chưa nhập số lượng và đơn giá hợp lệ cho bất kỳ sản phẩm nào.");
            setIsLoading(false);
            return;
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 transition-opacity">
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 m-4 max-h-[80vh] flex flex-col">
                ...
                <p className="text-sm text-gray-600 mb-6 border-b pb-4">
                    Nhà cung cấp: <span className="font-semibold text-green-700">{supplierName}</span>
                </p>
                
                {submitError && (
                    <p className="text-sm text-red-600 bg-red-50 p-3 rounded-lg mb-4">{submitError}</p>
                )}

                <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-hidden">
                    {/* Bảng sản phẩm (Scrollable) */}
                    <div className="flex-grow overflow-y-auto space-y-4 pr-2">
                        {isLoading && <Spinner variant="inline" size="sm" text="Đang tải sản phẩm..." />}
                        
                        {!isLoading && items.length === 0 && (
                            <div className="flex flex-col items-center justify-center text-center p-6 bg-gray-50 rounded-lg">
                                <AlertCircle className="w-12 h-12 text-gray-400 mb-2" />
                                <p className="font-medium text-gray-700">Không tìm thấy sản phẩm</p>
                                <p className="text-sm text-gray-500">Nhà cung cấp này chưa có sản phẩm nào trong kho.</p>
                            </div>
                        )}

                        {!isLoading && items.map(item => (
                            <div key={item.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center p-3 bg-gray-50 rounded-lg">
                                ...
                                <div>
                                    <label htmlFor={`item-qty-${item.id}`} className="text-xs font-medium text-gray-500">Số lượng nhập</label>
                                    <input
                                        type="number"
                                        id={`item-qty-${item.id}`}
                                ...
                                <div>
                                    <label htmlFor={`item-price-${item.id}`} className="text-xs font-medium text-gray-500">Đơn giá (VNĐ)</label>
                                    <input
                                        type="number"
                    ...
                    <div className="pt-6 mt-6 border-t flex justify-end gap-3">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <PackagePlus className="w-5 h-5 mr-2" />}
